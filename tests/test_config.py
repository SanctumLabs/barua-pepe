...
class TestConfigurationsCases(BaseTestCase):
    ...
    def test_app_has_set_sendgrid_token_and_url_to_test_values(self):
        """Test the application has set the SENDGRID_TOKEN and SENDGRID_URL to test values"""
        token = current_app.config.get("SENDGRID_TOKEN")
        base_url = current_app.config.get("SENDGRID_URL")
        self.assertEqual(token, "sendgrid-token")
        self.assertEqual(base_url, "https://api.sendgrid.test.com")
